...
public class BeanDefinitionParserDelegate {
	...
	public BeanDefinitionHolder parseBeanDefinitionElement(Element ele, @Nullable BeanDefinition containingBean) {
		 ...
		 * 中的 delegate 是 BeanDefinitionParserDelegate 的实例，
		 * 我们查看 BeanDefinitionParserDelegate 中的 decorateBeanDefinitionIfRequired 方法
		 */
	...
	public AbstractBeanDefinition parseBeanDefinitionElement(
		...
		try {
			/**
			 * 这里的parse开头的方法都是对bean定义的属性标签进行解析，例如「name」、「singleton」、「lazy-init」等，
			 * 大家可以自行深入了解每一个parse方法是如何解析各个属性的，在本文中就不再占用篇幅逐一讲解了。
			 * 至此我们已经获取到了BeanDefinition的信息，下一步就到「注册」了。
			 */
